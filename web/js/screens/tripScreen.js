...
function header(selected, list, days) {
  const first=days[0] || {};
  return el('header', { class:'trip-header' },
    el('div',{},
      el('p', { class:'eyebrow' }, 'YOUR TRIP TO'),
      dropdown(selected, [...list.map(t=>({value:t,label:t})), {value:'__new',label:'New Trip'}], v=> v==='__new' ? newTrip() : setState({selectedTripName:v}), 'Trip'),
      el('h1',{}, tripTitle(days) || 'Trip'),
      el('p',{}, tripRange(days)),
...
function timeline(days, selected) {
  ...
  return el('section', { class:'trip-timeline' },
    ...
    state.tripEdit ? el('div',{class:'candidate-row'}, button('➕ Add Day',()=>{},'btn'), candidates.map(c=>button(`${c.date.slice(5)} ${c.location}`,()=>addDayToTrip(c,selected),'chip-btn'))) : ''
  );
}
...
function dayItem(entry, last) {
  ...
  return el('article', { class:`trip-day ${expanded?'open':''}` },
    el('div',{class:'trip-date'}, el('b',{}, new Date(entry.date+'T00:00').toLocaleDateString(undefined,{weekday:'short'})), el('span',{},entry.date.slice(8))),
...
async function newTrip() {
  ...
  } else alert('No sheet row found for that date yet.');
}
