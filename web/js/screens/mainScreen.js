...
export function renderMainScreen() {
  ...
  const root = el('main', { class: 'screen main-screen' },
    ...
    el('div', { class: 'main-desktop-grid' },
      el('section', { class: 'main-left-pane' },
        header(),
        calendar(),
        scheduleList()
      ),
      el('section', { class: 'main-right-pane' },
        preview(selected)
      )
    ),
...
function header() {
  return el('section', { class: 'month-header compact-month-header' },
    el('button', {
      type: 'button',
      class: 'month-arrow',
      onClick: () => setState({ currentMonthOffset: state.currentMonthOffset - 1 })
    }, '‹'),
    el('button', {
      type: 'button',
      class: 'month-picker-button',
      onClick: () => setState({ modal: 'month-picker' })
    }, formatMonth(todayIso(), state.currentMonthOffset), el('span', { class: 'month-picker-caret' }, '▾')),
    el('button', {
      type: 'button',
      class: 'month-arrow',
      onClick: () => setState({ currentMonthOffset: state.currentMonthOffset + 1 })
...
function dayCell(day) {
  ...
  return el('button', {
      class: classes,
      type: 'button',
      onClick: () => setState({ selectedDate: day.iso })
    },
    emojis.length
  ...
  return el('section', { class: 'schedule-list' },
    ...
    el('div', { class: 'schedule-table' },
      ...
      entries.map(e => el('button', {
          class: e.date === state.selectedDate ? 'schedule-row selected' : 'schedule-row',
          type: 'button',
          onClick: () => setState({ selectedDate: e.date })
        },
...
function preview(entry) {
  ...
  return el('section', { class: 'preview-panel' },
    el('header', {
        class: 'preview-click-header',
        onClick: () => pushRoute('day')
      },
      el('div', {},
        el('h2', {}, formatLong(entry.date)),
        entry.location || entry.event
          ? el('p', {}, `${entry.location}${entry.location && entry.event ? ' • ' : ''}${entry.event}`.trim())
          : el('p', {}, 'Select a day')
      ),
